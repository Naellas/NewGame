...
final class StudyArmRig {
    ...
    StudyArmRig(BufferedImage source, int facing, int top, int body, double center, String name) {
        this.facing = facing; handLength = body * .045;
        boolean knightSide=name.startsWith("class_knight")&&name.endsWith("_model_right");
        boolean seraphineSide=name.startsWith("npc_seraphine")&&name.endsWith("_model_right");
        boolean[][] cloak=GroundedPartMasks.cloak(source,name);
        boolean mageSide = name.startsWith("class_mage") && name.endsWith("_model_right");
        ...
        double staffX = name.startsWith("class_cleric") && !name.equals("class_cleric_model_right") ? staffColumn(source, top, body, center) : Double.NaN;
        ...
        for (int i = 0; i < 2; i++) {
            ...
            chains[i] = new Chain(w, h, sx, sy, ex, ey, wx, wy,
                    (i == staffArm ? .02 : bowGrip ? .035 : samir || calder ? .09 : .16) * (facing == 0 ? .35 : 1));
        }
        owner = new int[h][w];
        for (int y = 0; y < h; y++) for (int x = 0; x < w; x++) {
            owner[y][x] = -1;
            int pixel = source.getRGB(x, y);
            if ((pixel >>> 24) < 32 || cloak[y][x] || (GroundedPartMasks.carryingPose(name)||name.equals("npc_calder_model_down"))) continue;
    ...
    Pose pose(int frame, int which) {
        ...
        double hand = lower + Math.sin(phase - .25) * .012;
