...
Usage:
    python tools/audio/musicgen.py
    python tools/audio/musicgen.py --list
    python tools/audio/musicgen.py --only zone_forest battle_boss --bars 24
    python tools/audio/musicgen.py --sample-pack
"""
...
from tools.audio.music_score import add_regional_presets, add_encounter_presets
...
def make_manifest(generated: Iterable[tuple[str, Path, TrackPreset, int]]) -> str:
    ...
    lines.append("")
    lines.append("Generated with `python tools/audio/musicgen.py`.")
    return "\n".join(lines)
...
def make_sample_manifest(
    ...
    command: str = "python tools/audio/musicgen.py --sample-pack",
) -> str:
    lines = [
        f"# {title}",
        "",
...
def main() -> None:
    ...
    if args.inspired_pack:
        ...
        generate_sample_pack(
            ...
            "python tools/audio/musicgen.py --inspired-pack",
        )
        return
    ...
    for name in names:
        ...
        if args.legacy:
            ...
            from tools.audio.music_score import render_score, write_score
    ...
    from tools.audio.music_score import write_encounter_player
