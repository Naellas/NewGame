...
from tools.assets.environments import extract_beach_assets as beach
from tools.assets.characters import extract_directional_character_assets as directional
from tools.assets.monsters import extract_goblin_monster_assets as goblins
from tools.assets.items import extract_item_icon_sheets as item_icons
from tools.assets.characters import extract_mage_directional_assets as mage
from tools.assets.monsters import extract_monster_expansion_assets as monsters
from tools.assets.characters import extract_new_npc_class_assets as npcs
from tools.assets.characters import extract_npc_variation_assets as npc_variations
from tools.assets.monsters import generate_monster_assets as base_monsters
from tools.assets.characters import generate_npc_assets as base_npcs
from tools.assets.environments import generate_player_model_assets as player_models
from tools.assets.characters import process_walk15_animations as walk15
from tools.assets.environments import slice_imagegen_village_assets as village
from tools.assets.items import slice_resource_props as resources
from tools.assets.shared.asset_paths import animation_dir, character_dir, player_dir
from tools.assets.shared.universal_cutout import touches_transparency


OUT_DIR = ROOT / "temp" / "cutout_tests" / "end_results" / "asset_replacement_proof"
...
def expected_assets() -> list[AssetSpec]:
    ...
    for name in beach.TERRAIN_NAMES:
        assets.append(AssetSpec(ROOT / "assets" / "environments/terrain/common" / f"{name}.png", "auto", allow_edge=True))
    for name in beach.PROP_NAMES:
        assets.append(AssetSpec(ROOT / "assets" / "environments/terrain/biomes/beach" / f"{name}.png", "magenta"))
    ...
    for row in goblins.GRID_NAMES:
        for name in row:
            assets.append(AssetSpec(ROOT / "assets" / "characters/monsters" / f"{name}.png", "green"))
    for row in monsters.GRID_NAMES:
        for name in row:
            assets.append(AssetSpec(ROOT / "assets" / "characters/monsters" / f"{name}.png", "green"))
    for row in base_monsters.GRID_NAMES:
        for name in row:
            assets.append(AssetSpec(ROOT / "assets" / "characters/monsters" / f"{name}.png", "magenta"))

    for name in npcs.NAMES:
        assets.append(AssetSpec(ROOT / "assets" / "characters/npcs/townsfolk" / f"{name}_model.png", "magenta"))
        assets.append(AssetSpec(ROOT / "assets" / "characters/npcs/townsfolk" / f"{name}.png", "magenta"))
