...
The smoke fixture for Aria's second chapter now accepts the quest through the normal action, invalidating the objective cache correctly. The latest wider `SmokeTest` run stops earlier at `assertVillageBuildingRoadConnections`: Dunewick reports zero connected building roads out of five. The full smoke suite is still not green; this pass does not change the world-generation code.
