...
public final class SmokeTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var workshopBase = state.world.buildingFootprints(WorldMap.PLAYER_VILLAGE_ID, workshop).get(0);
        if (PropCollision.clear(state.world, WorldMap.PLAYER_VILLAGE_ID, workshopBase.getCenterX(), workshopBase.getCenterY())) {
            throw new IllegalStateException("Village building core should block movement.");
        }
        if (!state.world.isPassable(WorldMap.PLAYER_VILLAGE_ID, workshop.x1() + 1, workshop.y1())) {
            throw new IllegalStateException("Village building roof overhang should leave a walkable back passage.");
        }
    ...
    private static void assertSettlementWeeklyQuestCoverage(GameState state) {
        ...
        Set<String> weeklyReasonLines = new HashSet<>();
        Set<String> weeklyDestinationKinds = new HashSet<>();
        int farmlandWeeklyQuestCount = 0;
        int weeklyQuestCount = 0;
        for (String kind : List.of("forest_shrine", "hidden_grove", "cave_mouth", "ruined_watchpost", "old_road_marker")) {
            if (state.world.locationSites(kind).isEmpty()) {
                throw new IllegalStateException("Missing generated quest destination patches for " + kind + ".");
            }
        }
        for (WorldMap.SettlementSite settlement : state.world.settlementSites()) {
            ...
            for (GameState.NpcMotion motion : motions) {
                Quest quest = state.questForNpc(motion.npc());
                if (quest == null || !quest.id.startsWith("weekly_")) {
                    continue;
                }
                weeklyQuestCount++;
                if (!quest.activeObjectiveLocationKind().isBlank()) {
                    weeklyDestinationKinds.add(quest.activeObjectiveLocationKind());
                    if ("farmland".equals(quest.activeObjectiveLocationKind())) {
                        farmlandWeeklyQuestCount++;
                    }
                }
        ...
        if (weeklyQuestCount >= 6 && weeklyOpeningLines.size() < 4) {
            throw new IllegalStateException("Weekly side quest openings lack variety: " + weeklyOpeningLines);
        }
        if (weeklyQuestCount >= 6 && weeklyReasonLines.size() < 3) {
            throw new IllegalStateException("Weekly side quest reason lines lack variety: " + weeklyReasonLines);
        }
        if (weeklyQuestCount >= 6 && weeklyDestinationKinds.size() < 5) {
            throw new IllegalStateException("Weekly side quest destinations lack variety: " + weeklyDestinationKinds);
        }
        if (weeklyQuestCount >= 6 && farmlandWeeklyQuestCount * 2 >= weeklyQuestCount) {
            throw new IllegalStateException("Too many weekly side quests point to farmland: "
                    + farmlandWeeklyQuestCount + "/" + weeklyQuestCount);
        }
    }

    private static void assertCityRaidDefenseQuest(GameState state) throws Exception {
        Quest quest = new Quest(
                "smoke_city_raid",
                "Smoke City Raid",
                "Smoke test raid defense quest.",
                "Riverside Raiders",
                1,
                0,
                0,
                Quest.ObjectiveKind.RAID_DEFENSE,
                "city_riverside",
                "city_riverside",
                0,
                "icon_shield",
                ...
                "Defend Riverside.",
                "Hold the line.",
                "Riverside held. Return.",
                "Riverside is safe."
        );
        quest.accepted = true;
        state.quests.put(quest.id, quest);
        state.mode = GameMode.EXPLORE;
        state.currentMapId = "city_riverside";
        state.playerX = Math.max(1, state.world.width("city_riverside") / 2);
        state.playerY = Math.max(1, state.world.height("city_riverside") / 2);
        state.startDefenseRaid();
        if (state.mode != GameMode.DEFENSE || state.defenseRaid == null) {
            throw new IllegalStateException("City raid defense did not start: " + state.status);
        }
        setPrivateBoolean(state.defenseRaid, "finished", true);
        setPrivateBoolean(state.defenseRaid, "victory", true);
        state.tickDefenseRaid();
        if (!quest.ready()) {
            throw new IllegalStateException("City raid defense victory did not advance the quest.");
        }
        state.closeDefenseRaid();
        state.quests.remove(quest.id);
    }
