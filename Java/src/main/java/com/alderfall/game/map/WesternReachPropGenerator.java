...
/** The western outward region, with deterministic, route-safe placement. */
final class WesternReachPropGenerator {
    private WesternReachPropGenerator() { }

    static void populate(WorldMap world) {
        place(world, "town_briarbridge", 21, 8, "village_prop_bench", 42, 14);
        place(world, "town_briarbridge", 27, 7, "village_prop_clay_oven", 48, 14);
        place(world, "town_briarbridge", 24, 10, "village_prop_seedling_tray", 36, 14);
        place(world, "city_riverside", 12, 19, "village_prop_bench", 42, 14);
        place(world, "city_riverside", 17, 12, "city_prop_cart", 48, 14);
        place(world, "village_foxbarrow", 8, 9, "village_prop_produce_basket", 38, 10);
        place(world, "village_foxbarrow", 6, 8, "village_prop_farm_tools", 38, 10);
        place(world, "village_foxbarrow", 22, 18, WesternReachFolklore.BOUNDARY, 56, 10);
        place(world, "dungeon_redcap_camp_1", 12, 10, WesternReachFolklore.CAMP_SUPPLIES, 72, 14);
        WorldMap.AdventureMarker redcap = world.adventureMarker("goblin_camp", 0);
        if (redcap != null && redcap.mapId().equals("dungeon_redcap_camp_1")) {
            place(world, WorldMap.OVERWORLD_ID, redcap.x() + 3, redcap.y() + 2,
                    WesternReachFolklore.CAMP_SUPPLIES, 64, 10);
        }

        // Follow the existing route and real settlement positions; no new quest markers.
        place(world, WorldMap.OVERWORLD_ID, 126, 127, WesternReachFolklore.BOUNDARY, 50, 10);
        place(world, WorldMap.OVERWORLD_ID, 87, 110, WesternReachFolklore.BOUNDARY, 50, 10);
        place(world, WorldMap.OVERWORLD_ID, 62, 152, WesternReachFolklore.BOUNDARY, 50, 10);
    }
    ...
    private static void place(WorldMap world, String mapId, int cx, int cy, String asset, int size, int radius) {
        ...
        if (!candidates.isEmpty()) {
            ...
            int padding = size > 80 ? 2 : size > 48 ? 1 : 0;
